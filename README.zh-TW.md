...
如果你在用 **Claude Code**、**Codex**、**OpenCode**、**PI** 或 **DeepSeek Harness** 這類「AI 程式設計助手」，你大概遇到過這些情況：
...
| | 能力 | 說明 |
| :---: | --- | --- |
| 📋 | **工作階段集中管理** | Claude Code、Codex、OpenCode、PI 和 DeepSeek Harness 的工作並排顯示，狀態一目了然：工作中、等待輸入、需要處理、已完成、失敗。 |
...
五個 Agent 可以同時開著，面板頂部的篩選按鈕能只看其中一家，或者「全部」一起看。
...
- **自動審批**：所有已連接 Agent 共用同一策略，可以手動逐個確認、只自動通過低風險命令，或自動通過全部審批；DSH 不提供持久化「一律允許」按鈕。
