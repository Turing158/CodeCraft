...
如果你在用 **Claude Code**、**Codex** 或 **OpenCode** 這類「AI 程式設計助手」，你大概遇到過這些情況：
...
| | 能力 | 說明 |
| :---: | --- | --- |
| 📋 | **工作階段集中管理** | Claude Code、Codex 和 OpenCode 的所有工作並排顯示，狀態一目了然：工作中、等待輸入、需要處理、已完成、失敗。 |
...
三個 Agent 可以同時開著，面板頂部的篩選按鈕能只看其中一家，或者「全部」一起看。
...
展開面板 → 按右上角 ⚙️ → **一般 → Hook 管理** → 按一下 Claude Code、Codex 或 OpenCode 完成安裝。

這一步在做什麼？CodeCraft 會往對應助手的設定裡加一個「通知掛鉤」，讓助手在開始工作、要呼叫工具、工作結束時主動告訴 CodeCraft 一聲。不裝它，面板會一直是空的。想撤銷隨時可以在同一處卸載，設定會被還原。

**3. 正常使用你的助手**

照常在終端機裡讓 Claude Code、Codex 或 OpenCode 做事。接下來工作階段卡片就會自己出現在面板上；有請求要處理時，面板會自動展開提醒你。
...
- **自動審批**：三個 Agent 共用同一策略，可以手動逐個確認、只自動通過低風險命令，或自動通過全部審批。
...
- 需要至少安裝 Claude Code、Codex 或 OpenCode 其中之一，CodeCraft 本身不包含 AI 模型，也不會替你呼叫任何 API
