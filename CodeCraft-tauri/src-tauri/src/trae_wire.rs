...
/// Trae's Chat and Solo send lifecycle/notification events without a project.
/// This exception is observation-only; it must never authorize a tool call.
pub fn workspace_less_observation(event: &str, value: &Value) -> bool {
    matches!(
        event,
        "SessionStart" | "UserPromptSubmit" | "Stop" | "Notification"
    ) && matches!(value["agent_type"].as_str(), Some("chat" | "solo_agent"))
        && value["cwd"] == "."
...
pub fn validate_hook(event: &str, value: &Value) -> Result<(), String> {
    ...
    if !workspace_less_observation(event, value) {
...
mod tests {
    ...
    fn names_and_error_outputs_are_not_heuristics() {
        assert_eq!(
            crate::protocol::own_tool("mcp__other__codecraft_ask_user"),
            None
        );
        assert_eq!(
