...
// These tools ask for a human response from inside their execute function.
// Let them reach that function so the resulting question event can be mirrored.
const CONTROL_TOOLS = new Set(["question", "plan_exit"]);
...
function baseDataDir() {
  if (process.env.CODECRAFT_MIMO_HOOK_DIR) {
    return path.resolve(process.env.CODECRAFT_MIMO_HOOK_DIR);
  }
  if (process.env.LOCALAPPDATA)
    return path.join(process.env.LOCALAPPDATA, "CodeCraft", "mimo-hook");
  return path.join(
...
async function atomicJson(directory, name, value) {
  ...
  if (Buffer.byteLength(serialized, "utf8") > MAX_ENVELOPE_BYTES) {
    throw new Error("CodeCraft Mimo IPC envelope exceeded its size limit");
  }
  writeSequence += 1;
...
function requestError(route, status, payload) {
  ...
  const error = new Error(
    `Mimo ${route} failed${status ? ` (HTTP ${status})` : ""}${detail ? `: ${clip(detail, 1000)}` : ""}`,
  );
  error.status = status;
  return error;
}
...
// The injected SDK client talks to the current Mimo instance even when it
...
async function httpRequest(method, route, body) {
  ...
  const headers = new Headers({
      "x-mimocode-directory": encodeURIComponent(context.directory),
  });
  ...
  try {
    ...
    throw new Error(
      `Mimo ${route} is unreachable at ${url.origin}: ${clip(String(error?.message ?? error), 500)}`,
    );
  }
  if (!response.ok) {
...
async function applyDecision(decision) {
  if (
    ...
    throw new Error("Invalid CodeCraft Mimo decision envelope");
  }
  ...
  if (decision.type === "plan") {
    const answer = decision.approved ? "Yes" : clip(decision.feedback, MAX_TEXT);
    await instanceRequest(
      "POST",
      "/question/{requestID}/reply",
      { requestID: decision.requestID },
  ...
  throw new Error("Unsupported CodeCraft Mimo decision type");
}
    ...
    waiter.reject(
      new Error(
        enabled
          ...
          : "CodeCraft Mimo integration was disabled",
      ),
    );
  }
}
...
async function eventPayload(event) {
  ...
  if (event?.type === "question.asked") {
    const question = Array.isArray(properties.questions)
      ...
      : undefined;
    const key = question?.key ?? question?.i18nKey;
    let planBody;
    if (key === "plan_exit") {
      const planPath = question?.params?.plan;
      if (typeof planPath === "string" && planPath.trim()) {
        try {
          const worktree = typeof context?.worktree === "string" ? context.worktree : undefined;
          const absolute = worktree ? path.resolve(worktree, planPath) : undefined;
          if (absolute && (!worktree || absolute === worktree || absolute.startsWith(`${worktree}${path.sep}`))) {
            planBody = clip(await readFile(absolute, "utf8"), MAX_TEXT);
          }
        } catch {
          planBody = undefined;
        }
...
async function codecraftMimoPlugin(input) {
  context = input;
    ...
    async event({ event }) {
      ...
      if (
        ...
        rejectGateWaitersForSession(
          sessionID,
          `Mimo session ended or released the tool gate (${event?.type})`,
        );
      }
      if (event?.type === "permission.asked") {
      ...
      if (event?.type === "question.asked") {
        await appendInbox("event", await eventPayload(event), 5);
        return;
      }
      await appendInbox("event", await eventPayload(event), 3);
    },
    "tool.execute.before": async (toolInput, output) => {
      ...
      if (nativeDecision && nativeDecision.sessionID === toolInput.sessionID) {
        if (nativeDecision.action === "reject")
          throw new Error("Mimo permission denied this tool call");
...
// Mimo loads every named module export as a plugin hook. Keep the runtime
...
export default Object.assign(codecraftMimoPlugin, {
  commandRisk,
