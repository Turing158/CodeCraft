use crate::{
    protocol::{self, DELIVERY_TOOL, ECHO_TOOL, MAX_NATIVE},
    store::{self, Client, Command, POLL},
};
...
fn error_result(message: String, operation: Option<&str>) -> CallToolResponse {
    let code = message.split(':').next().unwrap_or("STATE_UNAVAILABLE");
    let value = json!({"schemaVersion":1,"operationId":operation,"requestId":operation,"status":if code == "CANCELLED" {"cancelled"} else {"failed"},"payload":null,"error":{"code":code,"message":message,"retryable":false,"currentRevision":null}});
    if code == "CANCELLED" {
...
fn definition(name: &'static str) -> Tool {
    let schema = json!({"type":"object","additionalProperties":false,"required":["payload"],"properties":{"payload":{},"bridgeTicket":{"type":"string"}}});
    Tool::new(name, "CodeCraft P0 diagnostic only. Echoes nested payload; does not execute commands, answer production questions or approve production plans.", schema.as_object().unwrap().clone())
}
...
impl ServerHandler for ProbeServer {
    ...
    async fn list_tools(
        ...
        Ok(ListToolsResult {
            tools: vec![definition(ECHO_TOOL), definition(DELIVERY_TOOL)],
            ..Default::default()
        })
    }
    async fn call_tool(
        ...
        if request.name != ECHO_TOOL && request.name != DELIVERY_TOOL {
            return Err(ErrorData::invalid_params("Unknown probe tool", None));
        }
        let arguments = Value::Object(request.arguments.unwrap_or_default());
        if let Err(e) = protocol::validate_probe_args(&arguments) {
        ...
        guard.armed = false;
        Ok(CallToolResult::structured(json!({"schemaVersion":1,"operationId":operation,"requestId":operation,"status":"completed","payload":prepared["payload"],"error":null})).into())
    }
}
...
pub async fn serve(root: &Path) -> Result<(), String> {
    ...
    let connection = store::id();
    let service = ProbeServer {
        client: client.clone(),
    ...
    let mut output = tokio::spawn(async move {
        ...
        loop {
            ...
            let faults = if delivery.is_some() {
                let bytes =
                    std::fs::read(run_root.join("transport.json")).map_err(|e| e.to_string())?;
                protocol::strict_json(&bytes, 4096)?
            } else {
                json!({})
            };
            let before = faults["beforeWriteMs"].as_u64().unwrap_or(0).min(10_000);
            if before > 0 {
                tokio::time::sleep(std::time::Duration::from_millis(before)).await;
            }
            let mut out = stdout.lock().await;
            out.write_all(&line).await.map_err(|e| e.to_string())?;
            out.flush().await.map_err(|e| e.to_string())?;
            drop(out);
            let after = faults["afterFlushMs"].as_u64().unwrap_or(0).min(10_000);
            if after > 0 {
                tokio::time::sleep(std::time::Duration::from_millis(after)).await;
            }
            if let Some(d) = delivery {
                if faults["dropAck"] != true {
                    ...
                    .await
                    {
                        eprintln!("P0 delivery was flushed but not confirmed: {e}");
                    }
                }
            }
