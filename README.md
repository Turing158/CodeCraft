...
### WorkBuddy Hook-only 支持矩阵

| 能力 | WorkBuddy |
| --- | --- |
| 会话、提示、工具活动和原生结果 | 只读观察 |
| 工具权限、问题和计划请求 | 打开只读提醒，必须在 WorkBuddy 中处理 |
| 桌面端“前往 WorkBuddy 中处理” | 尝试激活已打开的 WorkBuddy 窗口；无法定位时显示原因 |
| LAN “前往 WorkBuddy 中处理” | 仅提示在运行 WorkBuddy 的设备上处理，不提供审批写接口 |
| 流式助手输出 | 当前协议未开放 |
